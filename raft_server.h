...
public:
  ...
  static const uint32_t timeout_ae_min = 2000 * 1000;
  static const uint32_t timeout_ae_max = 4000 * 1000;
  static const uint32_t timeout_hb = 500 * 1000;
  ...
  void send_rpc_ae_helper(raft_message_ae &rpc_ae, uint32_t next_index);

  void handle_rpc_ae(zmqpp::message &message);

  void send_reply_ae_accept(raft_message_ae &rpc_ae);

  void send_reply_ae_reject(raft_message_ae &rpc_ae);

  void handle_reply_ae(zmqpp::message &message);

  void handle_client_cmd(zmqpp::message &message);
